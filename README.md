...
This repo now also contains an in-progress Rust MVP under `Cargo.toml` and
`app/`. It embeds a WebView2 wallpaper host behind the desktop icons and
includes policy hooks to pause or resume wallpaper playback when:

- a fullscreen window becomes active
- a watched executable is running in a normal top-level window

Current Rust MVP commands:

```powershell
cargo run
```

The default wallpaper is `app/assets/wallpaper.html`.

The runtime config is created on first launch at:

```text
%APPDATA%\nolyn\weebp-rs\config\config.json
```
...
{
  "wallpaper": {
    "url": "file:///C:/Users/nolyn/weebp/app/assets/wallpaper.html"
  },
  "pause": {
...
The original C implementation below is still intact.
